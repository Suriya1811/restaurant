...
const TableTypeMaster = () => {
    ...
    const toggleSelectAll = () => {
        const currentIds = filteredTableTypes.map(tt => tt._id);
    ...
    const handleBulkAction = async (actionType) => {
        ...
        if (actionType === 'DELETE') {
            ...
            for (const id of selectedIds) {
                try {
                    await fetch(`${import.meta.env.VITE_API_URL}/table-types/${id}`, { method: 'DELETE', headers });
                } catch (err) { }
            }
            fetchData();
            ...
            for (const id of selectedIds) {
                try {
                    await fetch(`${import.meta.env.VITE_API_URL}/table-types/${id}`, { method: 'PUT', headers, body: JSON.stringify({ is_active: true }) });
                } catch (err) { }
            }
            fetchData();
            ...
            for (const id of selectedIds) {
                try {
                    await fetch(`${import.meta.env.VITE_API_URL}/table-types/${id}`, { method: 'PUT', headers, body: JSON.stringify({ is_active: false }) });
                } catch (err) { }
            }
            fetchData();
            ...
            for (const id of selectedIds) {
                try {
                    await fetch(`${import.meta.env.VITE_API_URL}/table-types/${id}`, { method: 'PUT', headers, body: JSON.stringify({ is_cancelled: true, is_active: false }) });
                } catch (err) { }
            }
            fetchData();
        }
                        ...
                        <div className="toolbar-premium">
                            ...
                            <div className="ml-auto flex-shrink-0">
                                <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-slate-50 border border-slate-200 text-slate-700 rounded-lg shadow-xs text-xs font-black uppercase tracking-wider" style={{ height: '32px' }}>
                                    ...
                                    <span className="text-sm font-black text-slate-900">{filteredTableTypes.length}</span>
                                </div>
                            </div>
                        </div>
                                ...
                                <thead>
                                    <tr>
                                        <th style={{ width: '40px', textAlign: 'center' }}>
                                            <input
                                                type="checkbox"
                                                checked={filteredTableTypes.length > 0 && filteredTableTypes.every(tt => selectedIds.includes(tt._id))}
