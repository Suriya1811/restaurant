...
const AccountsPayable = ({ isEmbedded = false }) => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const res = await fetch(`${import.meta.env.VITE_API_URL}/reports/aging-report?type=SUPPLIER`, { headers });
    ...
    const exportToCSV = () => {
        ...
        link.setAttribute("download", `Accounts_Payable_Aging.csv`);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };

    const totalPayable = Object.values(ranges).reduce((sum, val) => sum + val, 0);

    const chartData = {
        labels: ['0-30 Days', '30-60 Days', '60+ Days (Critical)'],
        datasets: [{
            label: 'Liability Volume (₹)',
        ...
        <div className="flex-1 flex flex-col min-h-0 bg-white">
            ...
            <div className="bg-white px-4 py-3 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm z-10">
                <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-slate-800 uppercase tracking-widest hidden sm:block">Accounts Payable</span>
                </div>
            ...
            <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/50">
                <div className="flex gap-4 overflow-x-auto custom-scrollbar">
                    ...
                    <div className="flex-1 min-w-[150px]">
                        <div className="text-xs text-slate-500 font-semibold mb-1">Total Liability</div>
                        <div className="text-xl font-bold text-red-600">₹{totalPayable.toLocaleString('en-IN', { maximumFractionDigits: 2 })}</div>
                    </div>
                </div>
            </div>
                            ...
                            <div className="overflow-auto flex-1 custom-scrollbar w-full">
                                <table className="w-full text-left border-collapse whitespace-nowrap min-w-[600px]">
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {details.length === 0 ? (
                                            <tr><td colSpan="4" className="text-center py-10 text-slate-400 font-bold bg-slate-50">No pending payables detected.</td></tr>
                                        ) : (
                                            details.map((item, ix) => (
    ...
    return (
        <div className="dashboard-layout bg-white">
            ...
            <main className="dashboard-main flex-1 overflow-hidden font-sans flex flex-col bg-slate-50">
                <Header toggleSidebar={toggleSidebar} title="Accounts Payable Summary" />
