...
const WaiterMaster = () => {
    ...
    const toggleSelectAll = () => {
        const currentIds = filteredWaiters.map(w => w._id);
    ...
    const handleBulkAction = async (actionType) => {
        ...
        if (actionType === 'DELETE') {
            ...
            for (const id of selectedIds) {
                try {
                    await fetch(`${import.meta.env.VITE_API_URL}/waiters/${id}`, { method: 'DELETE', headers });
                } catch (err) { }
            }
            fetchWaiters();
            ...
            for (const id of selectedIds) {
                try {
                    await fetch(`${import.meta.env.VITE_API_URL}/waiters/${id}`, { method: 'PUT', headers, body: JSON.stringify({ is_active: true }) });
                } catch (err) { }
            }
            fetchWaiters();
            ...
            for (const id of selectedIds) {
                try {
                    await fetch(`${import.meta.env.VITE_API_URL}/waiters/${id}`, { method: 'PUT', headers, body: JSON.stringify({ is_active: false }) });
                } catch (err) { }
            }
            fetchWaiters();
            ...
            for (const id of selectedIds) {
                try {
                    await fetch(`${import.meta.env.VITE_API_URL}/waiters/${id}`, { method: 'PUT', headers, body: JSON.stringify({ is_cancelled: true, is_active: false }) });
                } catch (err) { }
            }
            fetchWaiters();
        }
                        ...
                        <div className="toolbar-premium">
                            ...
                            <div className="ml-auto flex-shrink-0">
                                <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-slate-50 border border-slate-200 text-slate-700 rounded-lg shadow-xs text-xs font-black uppercase tracking-wider" style={{ height: '32px' }}>
                                    ...
                                    <span className="text-sm font-black text-slate-900">{filteredWaiters.length}</span>
                                </div>
                            </div>
                        </div>
                                ...
                                <thead>
                                    <tr>
                                        <th style={{ width: '40px', textAlign: 'center' }}>
                                            <input
                                                type="checkbox"
                                                checked={filteredWaiters.length > 0 && filteredWaiters.every(w => selectedIds.includes(w._id))}
                                    ...
                                    {loading ? (
                                        ...
                                        <tr key={waiter._id} className="group">
                                            <td className="w-10 text-center">
                                                <input
                                                    type="checkbox"
                                                    checked={selectedIds.includes(waiter._id)}
                                                    onChange={() => toggleSelectOne(waiter._id)}
                                                    className="w-4 h-4 rounded accent-[#ff6b00] cursor-pointer"
                                                />
                                            </td>
                                            <td className="w-10 text-center">
